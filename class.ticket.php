...
class Ticket {
   ...
   function onOverdue($whine=true, $comments="") {
    ...
    function update($vars, &$errors) {
        ...
        $fields=array();
        $fields['topicId']  = array('type'=>'int',      'required'=>1, 'error'=>'Tema de ayuda requerido');
        $fields['slaId']    = array('type'=>'int',      'required'=>0, 'error'=>'Seleccione SLA');
        $fields['duedate']  = array('type'=>'date',     'required'=>0, 'error'=>'Fecha invalida - debe ser MM/DD/YY');

        $fields['note']     = array('type'=>'text',     'required'=>1, 'error'=>'Razon por la que se actualizo el ticket');
        $fields['user_id']  = array('type'=>'int',      'required'=>0, 'error'=>'ID de usuario Invalido');

        if(!Validator::process($fields, $vars, $errors) && !$errors['err'])
            $errors['err'] = 'Info invalida o perdida - revise los errores e intente de nuevo';

        if($vars['duedate']) {
            if($this->isClosed())
                $errors['duedate']='Fecha de venc. no se puede poner en ticket cerrado';
            elseif(!$vars['time'] || strpos($vars['time'],':')===false)
                $errors['time']='Seleecion hora';
            elseif(strtotime($vars['duedate'].' '.$vars['time'])===false)
                $errors['duedate']='Fecha de venc. invalida';
            elseif(strtotime($vars['duedate'].' '.$vars['time'])<=time())
                $errors['duedate']='Fecha de venc. debe ser en el futuro12';
        }
    ...
    function create($vars, &$errors, $origin, $autorespond=true, $alertstaff=true) {
        ...
        if ($vars['email']  && Validator::is_email($vars['email'])) {
            ...
            if($cfg->getMaxOpenTickets()>0 && strcasecmp($origin,'staff')
                ...
                $errors['err']="Has alcanzado el maximo permitod en creacion de tickets.";
        ...
        switch (strtolower($origin)) {
            case 'web':
                $fields['topicId']  = array('type'=>'int',  'required'=>1, 'error'=>'Seleccion tema de ayuda');
                break;
            case 'staff':
                $fields['deptId']   = array('type'=>'int',  'required'=>1, 'error'=>'Dept. requerido');
                $fields['topicId']  = array('type'=>'int',  'required'=>1, 'error'=>'Topic requerido');
                $fields['duedate']  = array('type'=>'date', 'required'=>0, 'error'=>'Fecha de venc.invalida - de ser MM/DD/YY');
            case 'api':
                $fields['source']   = array('type'=>'string', 'required'=>1, 'error'=>'Seleccione origen');
                break;
            case 'email':
            ...
            default:
                ...
                $errors['err']=$errors['origin'] = 'El origen es invalido';
        }
        ...
        /*Genera el envio de datos a Slack */
        $sqlSlack = "INSERT INTO `ticket_slack` (`ticket_id`) VALUES ('$extId')";
        db_query($sqlSlack);
